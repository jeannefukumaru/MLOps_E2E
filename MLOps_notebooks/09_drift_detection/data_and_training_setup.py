...
project_home_dir = f"/Shared/DAIS_2021/"
...
workspace_project_home = f"/Shared/DAIS_2021"
...
raw_parquet_path = "dbfs:/tmp/airbnb-hawaii.parquet"
...
airbnb_df = spark.read.format("parquet").load(raw_parquet_path)
...
df_0.withColumn("month", F.lit("0")).write.format("delta").save(delta_path)
...
df_1_err.write.format("delta").mode("append").save(delta_path)
...
df_2_err = df_2.withColumn("price", F.col("price") + (2*F.col("price")*F.rand(seed=42))).withColumn("month", F.lit("2"))

df_2_err.write.format("delta").mode("append").save(delta_path)
...
def train_sklearn_rf_model(run_name, delta_path, model_params, misc_params, seed=42):
  ...
  with mlflow.start_run(run_name=run_name) as run:
    ...
    pdf = df.toPandas() 
    pdf = pdf[pdf["month"]==misc_params["month"]]
